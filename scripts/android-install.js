...
AndroidInstall.prototype.cloneConnectSDK = function () {
	...
	return Q.nfcall(fs.readdir, safePath('./cordova-plugin-connectsdk-chrome-cast'))
	.then(function (files) {
		for (var i = 0; i < files.length; i++) {
			if (files[i].indexOf('Connect-SDK-Android') !== -1) {
				csdkDirectory = files[i];
				return Q.nfcall(exec, commands.mv + " " + safePath("./cordova-plugin-connectsdk-chrome-cast/" + csdkDirectory) + " " + safePath("./csdk_tmp/" + csdkDirectory));
			}
		}
	})
	.then(function () {
		return Q.nfcall(exec, "git clone --depth 1 " + paths.ConnectSDK_Repository + " " + safePath("./cordova-plugin-connectsdk-chrome-cast/" + csdkDirectory));
	})
	.then(function () {
		return Q.nfcall(exec, "git checkout " + paths.ConnectSDK_Tag, {cwd: safePath("./cordova-plugin-connectsdk-chrome-cast/" + csdkDirectory)});
	})
	.then(function () {
		return Q.nfcall(exec, "git submodule update --init", {cwd: safePath("./cordova-plugin-connectsdk-chrome-cast/" + csdkDirectory)});
	})
	.then(function () {
		return Q.nfcall(exec, commands.cp + " " + safePath("../../plugins/cordova-plugin-connectsdk-chrome-cast/Connect-SDK-Android/build.gradle") + " " + safePath("./cordova-plugin-connectsdk-chrome-cast/" + csdkDirectory + "/build-extras.gradle"));
	})
	.then(function () {
		return Q.nfcall(exec, commands.cp + " " + safePath("./csdk_tmp/" + csdkDirectory + "/build.gradle") + " " + safePath("./cordova-plugin-connectsdk-chrome-cast/" + csdkDirectory + "/build.gradle"));
	});
};

AndroidInstall.prototype.revert_cloneConnectSDK = function () {
	console.log("Reverting Connect-SDK-Android repository clone");
	return Q.nfcall(exec, commands.rmRF + " " + safePath('./cordova-plugin-connectsdk-chrome-cast/' + csdkDirectory))
	.then (function () {
		return Q.nfcall(exec, commands.mv + " " + safePath("./csdk_tmp/" + csdkDirectory) + " " + safePath("./cordova-plugin-connectsdk-chrome-cast/" + csdkDirectory));
	})
};
		...
		response.pipe(file).on('close', function () {
			...
			uz.on('close', function () {
				if (deferred.promise.inspect().state !== "rejected") {
					...
					Q.nfcall(exec, commands.mv + " " + safePath(paths.AmazonFling_Jar) + " " + safePath("./cordova-plugin-connectsdk-chrome-cast/" + csdkDirectory + "/modules/firetv/libs/AmazonFling.jar"))
					.then(function () {
						console.log("Moving WhisperPlay.jar");
						return Q.nfcall(exec, commands.mv + " " + safePath(paths.WhisperPlay_Jar) + " " + safePath("./cordova-plugin-connectsdk-chrome-cast/" + csdkDirectory + "/modules/firetv/libs/WhisperPlay.jar"));
					})
					.then(function () {
						deferred.resolve();
					})
